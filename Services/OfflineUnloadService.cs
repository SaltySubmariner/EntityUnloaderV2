...
namespace OfflineUnload.Services
{
    public class OfflineUnloadService
    {
        ...
        public int Restore(ulong ownerId)
        {
            ...
            {
                File.Delete(path);
                Rocket.Core.Logging.Logger.Log("[OfflineUnload] Restore completed and JSON save was deleted.");
            }
            else
            {
        ...
        public bool ShouldBypassUnload(ulong ownerId)
        {
            if (ownerId == 0)
                return false;
            ...
            if (config.BypassAdmins && IsOnlineAdmin(ownerId))
                return true;

            var rocketPlayer = new SteamIdRocketPlayer(ownerId);

            if (HasAnyBypassPermission(rocketPlayer))
                return true;

            return IsInBypassGroup(rocketPlayer);
        }

        private bool IsOnlineAdmin(ulong ownerId)
        {
            try
            {
                var player = UnturnedPlayer.FromCSteamID(new CSteamID(ownerId));
                return player != null && player.IsAdmin;
            }
            catch (Exception ex)
            {
                Rocket.Core.Logging.Logger.LogWarning($"[OfflineUnload] Could not check admin bypass for {ownerId}: {ex.Message}");
                return false;
            }
        }

        private bool HasAnyBypassPermission(IRocketPlayer player)
        {
            var permissions = plugin.Configuration.Instance.BypassPermissions;
            if (permissions == null || permissions.Count == 0)
                return false;

            try
            {
                return R.Permissions.HasPermission(player, permissions);
            }
            catch (Exception ex)
            {
                Rocket.Core.Logging.Logger.LogWarning($"[OfflineUnload] Could not check Rocket permission bypass for {player.Id}: {ex.Message}");
                return false;
            }
        }

        private bool IsInBypassGroup(IRocketPlayer player)
        {
            var bypassGroupIds = plugin.Configuration.Instance.BypassGroupIds;
            if (bypassGroupIds == null || bypassGroupIds.Count == 0)
                return false;

            try
            {
                var groups = R.Permissions.GetGroups(player, true);
                return groups.Any(group => group != null && bypassGroupIds.Any(id => string.Equals(id, group.Id, StringComparison.OrdinalIgnoreCase)));
            }
            catch (Exception ex)
            {
                Rocket.Core.Logging.Logger.LogWarning($"[OfflineUnload] Could not check Rocket group bypass for {player.Id}: {ex.Message}");
                return false;
            }
        }

        private sealed class SteamIdRocketPlayer : IRocketPlayer
        {
            public SteamIdRocketPlayer(ulong steamId)
            {
                Id = steamId.ToString();
            }
